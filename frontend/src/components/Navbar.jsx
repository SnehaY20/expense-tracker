...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-6 left-0 right-0 z-50 w-full max-w-full px-0 flex items-center justify-center">
      ...
      <div className="absolute left-6 inset-y-0 flex items-center text-2xl font-extrabold tracking-tight bg-gradient-to-r from-purple-400 via-pink-400 to-orange-300 bg-clip-text text-transparent select-none">
        Expense
      </div>
      {/* Centered nav links container with rectangular rounded edges */}
      ...
      {/* LOGIN absolutely right, styled like nav links and vertically aligned */}
        ...
        <div className="px-6 py-4 space-y-4">
          {navLinks.map((section) => (
            <button
              key={section}
              onClick={() => handleNavClick(section.toLowerCase())}
              className={`block w-full text-left text-base font-semibold tracking-wide uppercase transition-colors transition-all duration-300 group/navlink ${
          ...
          <button
            className="block w-full text-left text-base font-semibold tracking-wide uppercase transition-colors transition-all duration-300 group/navlink cursor-pointer bg-none border-none shadow-none px-0 py-0 text-gray-400"
            style={{ letterSpacing: "0.08em", background: "none" }}
            onClick={() => (window.location.href = "/login")}
          >
            <span className="transition-all duration-300 group-hover/navlink:bg-gradient-to-r group-hover/navlink:from-purple-400 group-hover/navlink:via-pink-400 group-hover/navlink:to-orange-300 group-hover/navlink:bg-clip-text group-hover/navlink:text-transparent">
